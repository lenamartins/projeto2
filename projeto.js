document.addEventListener("DOMContentLoaded", () => {
  ...
  const locais = [
    ...
    },
    {
      ...
      gps: "https://www.google.com/maps/place/Catedral+S%C3%A3o+Paulo+Ap%C3%B3stolo/@-26.9196398,-49.0687201,17z",
    ...
    },
    {
      ...
      endereco: "Praça Victor Konder, 2, Centro, Blumenau - SC",
    ...
    },
    {
      ...
      descricao: "Museu de ciências naturais com acervo de taxidermia, biblioteca etc. Possui rampas, banheiros adaptados e acervos em Braile.",
    ...
    },
    {
      ...
      descricao: "O MAB promove visitas sensoriais para pessoas com deficiência visual, permitindo explorar obras pelo toque e interação, e visitas mediadas adaptadas.",
    ...
    lista.forEach((local, idx) => {
      ...
      card.innerHTML = `
        ...
        <div class="card-body">
          ...
          <div class="card-description">${local.descricao}</div>
          <div class="card-buttons">
            <button class="btn-secondary" onclick="openModal(${idx})">Ver Detalhes</button>
            <a href="${local.gps}" class="btn-gps" target="_blank">Abrir no Google Maps</a>
          </div>
        </div>
      `;
  ...
  filterOptions.forEach(option => {
    option.addEventListener("click", () => {
      ...
      const filteredLocais = locais.filter(local => local.tags.includes(filterType));
      renderCards(filteredLocais);
    });
  });
  ...
  function openModal(index) {
    ...
    modalTitle.innerHTML = local.nome;
    modalImg.src = local.img;
    modalAddress.innerHTML = `Endereço: ${local.endereco}`;
    modalPhone.innerHTML = `Telefone: ${local.telefone}`;
    modalDescription.innerHTML = local.descricao;
    modalRating.innerHTML = `Avaliação: ${local.rating ? local.rating.toFixed(1) + " ★" : "Sem avaliação"}`;

    modalTags.innerHTML = local.tags.map(tag => `<span>${tagLabel(tag)}</span>`).join("");
  ...
  // Exibir todos os locais ao iniciar
  renderCards(locais);
});
